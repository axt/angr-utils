...
def get_signed_range(se, expr):
    ...
    if not sat_zero(se, expr):
        try: 
            ...
            pass
        try: 
            smin = -(1 << size) + se.min(expr, extra_constraints=[se.Extract(size-1,size-1,expr) == 1])
            smax = -(1 << size) + se.max(expr, extra_constraints=[se.Extract(size-1,size-1,expr) == 1])
            return (smin, smax)
        except:
            pass
        return None
...
def sat_negative(se, expr):
    return se.satisfiable(extra_constraints=([se.Extract(size-1,size-1,expr) == 1]))

def sat_positive(se, expr):
    return se.satisfiable(extra_constraints=([se.Extract(size-1,size-1,expr) == 0]))
