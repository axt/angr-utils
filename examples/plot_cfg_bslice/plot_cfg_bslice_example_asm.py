...
if __name__ == "__main__":
    ...
    vis = AngrVisFactory().default_cfg_pipeline(project, asminst=True, vexinst=False)
    vis.set_output(DotOutput('cfg_asm'))
    ...
    vis = AngrVisFactory().default_cfg_pipeline(project, asminst=True, vexinst=False)
    vis.set_output(DotOutput('cfg_asm_annotated'))
    vis.add_content_annotator(AngrBackwardSliceAnnotatorAsm(bs))
    vis.process(cfg.graph) 
